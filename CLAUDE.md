...
- **JS-side SQL transactions are unsound**: tauri-plugin-sql runs every `execute` on an arbitrary sqlx pool connection, so `withTransaction`'s BEGIN/COMMIT can land on different connections and break under concurrent load (this silently emptied the vault: materialization failed after its cache write, and the fresh stateKey blocked retries). `services/records` uses sequential autocommit statements with retry-after-failure ordering instead; `imapSync`/`snoozeManager` still use `withTransaction` and carry the same latent risk under load
