...
2. **Service layer** (`src/services/`): All business logic. Plain async functions (not classes, except `GmailClient`).
   ...
   - `ledger/` — obligations pipeline (`candidates`, `extractor`, `ledger`, `ledgerManager`, `obligationLines`, `nudge`): deterministic sent-mail candidates → cached `ledger_extract_v1` extractions in `ai_cache` → entries derived at read time; `ledger_overrides` stores dismiss/done/pin; the Brief weaves in top obligations.
